...
const int trig = 3;
const int echo = 4;
const int motor_11 = 5; //motot 1
const int motor_12 = 6;
int ENA = 7;
const int motor_21 = 11;//motor 2
const int motor_22 = 13;
int ENB = 12;
const int motor_31 = 8;//motor3
const int motor_32 = 9;
int ENC = 10;

const int led= 2;//light
const int solinoid_v = 0;//spray machine
...
void loop() {
  ...
  distance = calculateDistance(trig, echo); // measure distance using sonar
  //  for (;;) {
...
int controlMotor_two_L2R() {
  ...
  // on 4 second
  const auto finalTime = millis() + 4 * 1000;
...
int Stop() {
  ...
  for (i = 0; i < step_count-1; i++) {
    ...
    digitalWrite(motor_32, HIGH);
	  delay(500);
    Serial.print("climb motor 3");
	  Serial.print(i);
  }
  digitalWrite(motor_31, LOW);
  digitalWrite(motor_32, LOW);
  tone(buzzer, 1500, 1000); // buzzer sound
  delay(1000000); //stop motor_3 for long time
}
